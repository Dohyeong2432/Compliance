...
- **목록 → 본문 페이지 이동**: 실제 사용 중이라고 확인해주신 참고 코드
  ...
  - `open_law_detail_by_name(browser, site_category, law_name)` — 이름을
    ...
    **law.go.kr 자체 검색(`query=`)으로 먼저 좁힌 결과 목록 안에서 찾습니다**
    ...
    - **같은 법령명이 여러 행으로 나올 때**: 검색 결과에 현행 버전과, 이미
      공포됐지만 아직 시행 전인 개정 버전이 이름은 똑같이 나란히 뜰 수
      있습니다(실제 "자본시장과 금융투자업에 관한 법률" 검색 결과로 확인함).
      law.go.kr는 아직 시행 전인 행에만
      `<img alt="앞으로 시행될 법령">`을 붙여서 구분해주므로, `_parse_listing_table`이
      이걸 `_upcoming` 플래그로 같이 뽑고, `open_law_detail_by_name`은 정확히
      일치하는 행이 여럿이면 `_upcoming`이 아닌(=현행) 행을 우선 선택합니다.
    - **`get_page_range`의 경계값 버그**: 검색 결과가 딱 1페이지(흔한
      경우)일 때 `last_page_number=1`이 되는데, 원래 코드는 이 경우 빈
      리스트를 반환해서 그 유일한 페이지조차 확인하지 않고 "못 찾았다"고
      끝나버렸습니다(`last_page_number`가 1, 6, 11, ...처럼 새 5개 구간의
      시작점과 같은 값일 때 그 마지막 구간이 통째로 빠지는 경계값 문제).
      `range()`의 stop을 `last_page_number + 1`로 고쳐 해결.
