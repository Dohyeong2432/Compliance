...
**회수(recall) 폭과 최종 노출(top_k)은 별개 값입니다.** `_recall_candidates()`가
BM25/벡터 각 채널에서 실제로 요청하는 폭은 `top_k`가 아니라
`top_k * _RECALL_WIDTH_MULTIPLIER`(기본 5배)입니다. 처음엔 두 값이 같았는데,
실사용에서 이 문제가 발견됐습니다: `source_types`를 좁혀도(예: 법령만 남겨도)
채널 내 순위가 `top_k`등 밖인 문서는 애초에 회수 단계에서 걸러져 RRF 융합
후보에도 못 들어가, 다른 소스와 "경쟁해서 진" 게 아니라 "경쟁할 기회조차
못 얻는" 상황이 실제로 발생했습니다(어휘적으로 사규가 더 가깝게 매치되는
질의에서 법령이 top_k=6 회수 폭 안에 전혀 안 들어온 사례로 확인). 회수를
넓게, 최종 컷은 리랭킹 단계(`retrieve()`의 `self.reranker.rerank(..., top_k)`)
한 곳에서만 하도록 분리했습니다 — 이게 원래 이 모듈 docstring이 6단계에서
말하던 "20-30개 후보 풀"의 의도였습니다.

**회수 폭을 넓혀도 남는 문제가 하나 더 있었습니다: 최종 컷 자체가 "전체 풀
상위 top_k" 단일 기준이라, 개체 수가 많은 소스가 융합 순위 상위를 통째로
차지하면 개체 수가 적은 소스를 완전히 밀어냅니다.** 실사용에서 재현된 사례:
사규는 조문 단위로 세밀하게 쪼개져 있어 법령보다 개체 수가 훨씬 많고, 특정
질의에서 사규 6건이 융합 순위 1~6등을 모두 차지해 법령(7등)이 회수 폭을
넓힌 뒤에도 여전히 `top_k=6` 밖으로 잘려나갔습니다. `_stratify_by_source()`가
이걸 해결합니다: 최종 후보는 "flat top_k 컷"과 "소스(`entity_type`)별 상위
`_MIN_PER_SOURCE_TYPE`개(기본 5)"의 합집합입니다 — 사규가 아무리 많아도
법령을 완전히 밀어내지 못한다는 구조적 보장입니다. `entity_types` 필터가
이미 단일 타입으로 좁혀진 경우(예: LLM이 `source_types=["law"]`로 호출)는
소스 간 경쟁 자체가 없으므로 이 계층화를 건너뛰고 기존 flat `top_k` 컷만
씁니다. 이 합집합을 리랭커에 넘길 때는 `top_n`을 합집합 크기와 동일하게
줍니다(`self.reranker.rerank(query, texts, len(selected))`) — `NoOpReranker`/
`VoyageReranker` 모두 `top_n == len(documents)`이면 입력을 전량 보존한 채
순서만 재조정하므로(`knowledge/reranker.py`), 리랭킹 단계에서 이 소스별
최소 보장이 다시 깨지지 않습니다. 부작용: 최종 반환 문서 수가 더 이상
`top_k`로 고정되지 않고(등장하는 소스 타입 수에 비례해 늘어날 수 있음),
LLM에 전달되는 컨텍스트도 그만큼 커집니다 — 이는 "top_k를 무작정 올리는
것보다, 소스별 최소 보장이 더 근본적인 해법"이라는 판단에 따른 의도된
트레이드오프입니다.
